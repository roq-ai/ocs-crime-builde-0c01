...
import { getPlayerKnowledgeById, updatePlayerKnowledgeById } from 'apiSdk/player-knowledges';
...
function PlayerKnowledgeEditPage() {
  ...
  const handleSubmit = async (values: PlayerKnowledgeInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updatePlayerKnowledgeById(id, values);
      mutate(updated);
      resetForm();
