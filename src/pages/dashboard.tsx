...
function DashboardPage() {
  ...
  const knowledgeStat = useSWR<PaginatedInterface<KnowledgeInterface>>(
    () => `/knowledges`,
    () => getKnowledges({ limit: 1, offset: 0 }),
  );

  const crimeStat = useSWR<PaginatedInterface<CrimeInterface>>(
    () => `/crimes`,
    () => getCrimes({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Knowledges',
      result: knowledgeStat?.data?.totalCount,
      isLoading: knowledgeStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Crimes',
      result: crimeStat?.data?.totalCount,
      isLoading: crimeStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Players',
